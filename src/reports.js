...
import { buildChartData, DARK_CSS, CLIENT_RUNTIME } from './charts.js';
...
export function basic(returns, title = 'Portfolio Performance Report', rfRate = 0, nans = false, benchmark = null, benchmarkTitle = 'Benchmark') {
  ...
  const chartData = buildChartData(normalizedReturns, normalizedBenchmark, benchmarkTitle);
  chartData.title = title;
  // heatmap height = one compact row per year + chrome (task 11)
  const heatH = Math.max(180, chartData.heatmap.years.length * 34 + 120);

  // --- panel wrappers: every chart/section becomes a draggable, collapsible
  // panel with a stable data-panel-id. The client runtime reorders these and
  // toggles `.collapsed` from a shared localStorage layout. ---
  const drag = '<span class="drag" title="Drag to reorder">⠿</span>';
  const collapseBtn = '<button class="collapse" title="Collapse / expand">▾</button>';
  const chartPanel = (id, label, cls = '', style = '') =>
    `<div class="panel" data-panel-id="${id}">
      <div class="panel-head">${drag}<span class="ptitle">${label}</span>${collapseBtn}</div>
      <div class="panel-body"><div class="chart${cls}" id="chart_${id}"${style}></div></div>
    </div>`;
  const sectionPanel = (id, label, inner) =>
    `<div class="panel" data-panel-id="${id}">
      <div class="panel-head">${drag}<span class="ptitle">${label}</span>${collapseBtn}</div>
      <div class="panel-body">${inner}</div>
    </div>`;

  // Default left-column order: monthly-returns heatmap, then worst drawdown
  // periods, then the rest unchanged. Users can reorder; their saved order wins.
  const leftPanels = [
    chartPanel('heatmap', 'Monthly Returns (%)', '', ` style="height:${heatH}px"`),
    chartPanel('ddPeriods', 'Worst Drawdown Periods', ' tall'),
    chartPanel('cumulative', 'Cumulative Returns'),
    chartPanel('log', 'Cumulative Returns (Log)'),
    normalizedBenchmark ? chartPanel('rollingBeta', 'Rolling Beta') : '',
    chartPanel('eoy', 'EOY Returns'),
    chartPanel('monthlyDist', 'Monthly Returns Distribution'),
    chartPanel('daily', 'Daily Returns'),
    chartPanel('rollingVol', 'Rolling Volatility'),
    chartPanel('rollingSharpe', 'Rolling Sharpe'),
    chartPanel('rollingSortino', 'Rolling Sortino'),
    chartPanel('underwater', 'Underwater Plot'),
    chartPanel('quantiles', 'Return Quantiles'),
  ].join('');

  // Default right-column order: CAGR summary, end-of-year returns, worst 30
  // drawdowns, then the full key-metrics tables last.
  const rightPanels = [
    sectionPanel('summary', 'Summary', generateHero(performanceMetrics)),
    sectionPanel('eoyTable', 'End of Year Returns', generateEOYTable(normalizedReturns, normalizedBenchmark, benchmarkTitle)),
    sectionPanel('ddTable', 'Worst 30 Drawdowns', generateDrawdownTable(normalizedReturns)),
    sectionPanel('keyMetrics', 'Key Performance Metrics', generateMetricsSections(performanceMetrics, benchmarkMetrics, benchmarkTitle)),
  ].join('');
...
<html lang="en">
  <head>
    ...
    <title>${title} — Tearsheet (QuantStats.js)</title>
  ...
  <body>
    <div class="container">
      <div class="header">
        <h1>${title} <dt>${dateRange}</dt></h1>
        <h4>Generated by QuantStats.js — dark interactive tearsheet · <a href="#" id="qsResetLayout">Reset layout</a></h4>
      </div>
      <div class="content">
        <div id="left">${leftPanels}</div>
        <div id="right">${rightPanels}</div>
      </div>
    ...
    <script>window.__QS_DATA__ = ${JSON.stringify(chartData)};</script>
...
function generateEOYTable(returns, benchmark = null, benchmarkTitle = 'Benchmark') {
  ...
  try {
    ...
    let eoyCum = 1; // running compounded cumulative (no-benchmark column)
    ...
    sortedYears.forEach(year => {
      ...
      if (benchmark) {
        ...
        eoyCum *= (1 + strategyTotalReturn);
        const ret = strategyTotalReturn * 100;
        const cum = (eoyCum - 1) * 100;
    ...
    const headers = benchmark
      ...
      : `<tr><th>Year</th><th>Return</th><th>Cumulative</th></tr>`;
...
function generateDrawdownTable(returns) {
  ...
  try {
    ...
    sortedDrawdowns.forEach((dd) => {
      const startDate = dd.start || 'N/A';
      const recovered = dd.end || 'Ongoing';
      const duration = dd.days ? `${dd.days} days` : 'Ongoing';
      const maxDD = dd['max drawdown'] != null ? dd['max drawdown'].toFixed(2) + '%' : 'N/A';

      // data-start/data-end (ISO) let the runtime shade this period on the
      // "Worst Drawdown Periods" chart when the row is hovered.
      tableRows += `<tr class="dd-row" data-start="${dd.start || ''}" data-end="${dd.end || ''}">
        <td>${startDate}</td>
        <td>${recovered}</td>
        <td>${duration}</td>
        <td class="neg">${maxDD}</td>
      </tr>\n`;
    });
    ...
    return `<table>
      <thead>
        <tr><th>Started</th><th>Recovered</th><th>Duration</th><th>Max DD</th></tr>
      </thead>
      <tbody>
        ${tableRows}
      </tbody>
...
/* ===================================================================
 * Dark tearsheet: hero block, sectioned collapsible metrics, tooltips
 * =================================================================== */

// metric label → plain-English explanation (task 7)
const METRIC_INFO = {
  'Cumulative Return %': 'Total compounded growth over the whole period.',
  'CAGR%': 'Compound annual growth rate — the smoothed yearly return.',
  'Sharpe': 'Excess return per unit of total volatility. Higher is better; >1 is good.',
  'Smart Sharpe': 'Sharpe adjusted for autocorrelation (smoother streaks inflate raw Sharpe).',
  'Prob. Sharpe Ratio %': 'Probability the true Sharpe is above 0 given the sample.',
  'Sortino': 'Like Sharpe but only penalizes downside volatility.',
  'Smart Sortino': 'Sortino adjusted for autocorrelation.',
  'Sortino/√2': 'Sortino divided by √2 to compare directly against Sharpe.',
  'Calmar': 'CAGR divided by max drawdown — return per unit of worst loss.',
  ...
  'Common Sense Ratio': 'Profit factor × tail ratio — rewards fat right tails.',
  'CPC Index': 'Profit factor × win rate × payoff ratio.',
  'Tail Ratio': 'Right-tail size vs left-tail size; >1 means bigger gains than losses.',
  'Outlier Win Ratio': 'Size of the biggest wins vs the median win.',
  'Outlier Loss Ratio': 'Size of the biggest losses vs the median loss.',
  'Recovery Factor': 'Total return divided by max drawdown — recovery strength.',
  'Serenity Index': 'Return adjusted for ulcer (drawdown pain) and tail risk.',
  'Volatility (ann.) %': 'Annualized standard deviation of returns.',
  'R²': 'Fit of returns to a linear trend; how steady the path is.',
  'Skew': 'Asymmetry of returns. Positive favors large gains, negative large losses.',
  'Kurtosis': 'Fatness of the tails — high means more extreme moves than normal.',
  'Daily Value-at-Risk %': 'Loss not expected to be exceeded on 95% of days.',
  'Expected Shortfall (cVaR) %': 'Average loss on the worst 5% of days.',
  'Kelly Criterion %': 'Optimal fraction of capital to bet for growth.',
  'Risk of Ruin %': 'Probability of losing the whole account.',
  'Ulcer Index': 'Depth and duration of drawdowns combined.',
  'Max Drawdown %': 'Largest peak-to-trough drop in value.',
  'Longest DD Days': 'Longest time spent below a prior peak.',
  'Avg. Drawdown %': 'Average size of drawdowns.',
  'Avg. Drawdown Days': 'Average length of drawdowns.',
  'Win Days %': 'Share of days with positive return.',
  'Win Month %': 'Share of months with positive return.',
  'Win Quarter %': 'Share of quarters with positive return.',
  'Win Year %': 'Share of years with positive return.',
  'Avg. Up Month': 'Average return in winning months.',
  'Avg. Down Month': 'Average return in losing months.',
  'Max Consecutive Wins': 'Longest streak of winning periods.',
  'Max Consecutive Losses': 'Longest streak of losing periods.',
  'Expected Daily %': 'Average daily return.',
  'Expected Monthly %': 'Average monthly return.',
  'Expected Yearly %': 'Average yearly return.',
  'Time in Market %': 'Fraction of time holding a position.',
  'Risk-Free Rate %': 'Baseline return subtracted when computing excess return.',
};

// ordered sections (task 6). Keys not listed fall into "Other".
const METRIC_SECTIONS = [
  { name: 'Overview', keys: ['Start Period', 'End Period', 'Risk-Free Rate %', 'Time in Market %'] },
  { name: 'Returns', keys: [
    'Cumulative Return %', 'CAGR%', 'Expected Daily %', 'Expected Monthly %', 'Expected Yearly %',
    'MTD %', '3M %', '6M %', 'YTD %', '1Y (ann.) %', '3Y (ann.) %', '5Y (ann.) %', '10Y (ann.) %',
    'All-time (ann.) %', 'Best Day %', 'Worst Day %', 'Best Month %', 'Worst Month %', 'Best Year %', 'Worst Year %'] },
  { name: 'Risk-Adjusted Ratios', keys: [
    'Sharpe', 'Smart Sharpe', 'Prob. Sharpe Ratio %', 'Sortino', 'Smart Sortino', 'Sortino/√2',
    'Calmar', 'Gain/Pain Ratio', 'Payoff Ratio', 'Profit Factor', 'Common Sense Ratio', 'CPC Index',
    'Tail Ratio', 'Outlier Win Ratio', 'Outlier Loss Ratio', 'Recovery Factor', 'Serenity Index'] },
  { name: 'Risk', keys: [
    'Volatility (ann.) %', 'R²', 'Skew', 'Kurtosis', 'Daily Value-at-Risk %', 'Expected Shortfall (cVaR) %',
    'Kelly Criterion %', 'Risk of Ruin %', 'Ulcer Index'] },
  { name: 'Drawdown', keys: ['Max Drawdown %', 'Longest DD Days', 'Avg. Drawdown %', 'Avg. Drawdown Days'] },
  { name: 'Win / Loss', keys: [
...
function infoIcon(key) {
  const txt = METRIC_INFO[key];
  return txt ? ` <span class="info" title="${txt.replace(/"/g, '&quot;')}">&#9432;</span>` : '';
}

// big stat block, top-right (task 5)
function generateHero(metrics) {
  const subs = [
    ['Total Return', metrics['Cumulative Return %']],
    ['Max Drawdown', metrics['Max Drawdown %']],
    ['Longest DD Days', metrics['Longest DD Days']],
  ...
  return `<div class="hero">
    <div class="metric-main">
      <div class="t">CAGR${infoIcon('CAGR%')}</div>
      <h1 class="${valueClass(cagr)}">${cagr}</h1>
    </div>
    <div class="metric-sub-grid">
      ${subs.map(([label, val]) => `<div class="metric-sub">
        <div class="t">${label}</div>
        <h2 class="${valueClass(val)}">${val ?? '-'}</h2>
      </div>`).join('')}
    </div>
  </div>`;
}

// collapsible labeled metric sections (tasks 6 + 7)
function generateMetricsSections(metrics, benchmarkMetrics = null, benchmarkTitle = 'Benchmark') {
  const used = new Set();
  const hasBench = benchmarkMetrics !== null;
  const head = hasBench
    ? `<tr><th>Metric</th><th>Strategy</th><th>${benchmarkTitle}</th></tr>`
    : `<tr><th>Metric</th><th>Strategy</th></tr>`;

  const renderRows = (keys) => keys
    .filter((k) => metrics[k] !== undefined && k.trim() !== '')
    .map((k) => {
      used.add(k);
      const v = metrics[k];
      const bench = hasBench ? `<td class="${valueClass(benchmarkMetrics[k])}">${benchmarkMetrics[k] ?? '-'}</td>` : '';
      return `<tr><td>${k}${infoIcon(k)}</td><td class="${valueClass(v)}">${v ?? '-'}</td>${bench}</tr>`;
    }).join('');

  let out = '';
  for (const sec of METRIC_SECTIONS) {
    const rows = renderRows(sec.keys);
    if (!rows) continue;
    out += `<details open><summary>${sec.name}</summary>
      <div class="table-wrapper"><table><thead>${head}</thead><tbody>${rows}</tbody></table></div>
    </details>`;
  }
  // catch-all for any metric not assigned to a section
  const leftover = Object.keys(metrics).filter((k) => !used.has(k) && k.trim() !== '' && metrics[k] !== undefined);
  if (leftover.length) {
    out += `<details><summary>Other</summary>
      <div class="table-wrapper"><table><thead>${head}</thead><tbody>${renderRows(leftover)}</tbody></table></div>
    </details>`;
  }
  return out;
}
